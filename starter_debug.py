...
def compute_loss(
    ...
    # DEBUG PRINTS
    print(f"[DEBUG compute_loss] advantages shape: {advantages.shape}, min: {advantages.min():.6f}, max: {advantages.max():.6f}, mean: {advantages.mean():.6f}")
    print(f"[DEBUG compute_loss] policy_log_probs: min: {policy_log_probs.min():.6f}, max: {policy_log_probs.max():.6f}")
    print(f"[DEBUG compute_loss] pi_ratio: min: {pi_ratio.min():.6f}, max: {pi_ratio.max():.6f}, mean: {pi_ratio.mean():.6f}")
    print(f"[DEBUG compute_loss] unclipped_term: min: {unclipped_term.min():.6f}, max: {unclipped_term.max():.6f}")
    
    # PPO loss is the negative minimum of the unclipped and clipped terms
    loss = -torch.minimum(unclipped_term, clipped_term)
    
    print(f"[DEBUG compute_loss] loss_per_token: min: {loss.min():.6f}, max: {loss.max():.6f}, mean: {loss.mean():.6f}")
    
    # The function signature requires a tuple of (loss, metadata_dict).
    # We return the loss and an empty dictionary as the metadata.
    metadata = {}
...
def masked_mean(tensor: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    ...
    # DEBUG PRINTS
    print(f"[DEBUG masked_mean] tensor shape: {tensor.shape}, mask sum: {mask.sum()}")
    print(f"[DEBUG masked_mean] row_sums (first 5): {row_sums[:5]}")
    print(f"[DEBUG masked_mean] row_counts (first 5): {row_counts[:5]}")
    print(f"[DEBUG masked_mean] row_means (first 5): {row_means[:5]}")
    print(f"[DEBUG masked_mean] final mean: {row_means.mean():.6f}")
...
def grpo_microbatch_step(
    ...
    # DEBUG PRINTS
    print(f"[DEBUG microbatch] loss_per_token shape: {loss_per_token.shape}")
    print(f"[DEBUG microbatch] response_mask shape: {response_mask.shape}, sum: {response_mask.sum()}")
    ...
    print(f"[DEBUG microbatch] loss BEFORE division: {loss.item():.6f}")
    
    loss = loss / gradient_accumulation_steps
    loss.backward()
    
    print(f"[DEBUG microbatch] loss AFTER division: {loss.item():.6f}")
...
def train(
    ...
    for _ in range(n_grpo_steps):
        ...
        # DEBUG PRINTS
        print(f"[DEBUG train] advantages shape: {advantages.shape}, min: {advantages.min():.6f}, max: {advantages.max():.6f}")
        print(f"[DEBUG train] Number of microbatches: {rollout_batch_size // micro_train_batch_size}")
        
        for micro_idx in range(0, rollout_batch_size, micro_train_batch_size):
            s = slice(micro_idx, micro_idx + micro_train_batch_size)
            print(f"[DEBUG train] Processing microbatch {micro_idx//micro_train_batch_size + 1}, slice: {micro_idx}:{micro_idx + micro_train_batch_size}")
            ...
            print(f"[DEBUG train] Microbatch loss returned: {loss.item():.6f}")
            rollout_loss += float(loss.item())
            print(f"[DEBUG train] Accumulated rollout_loss: {rollout_loss:.6f}")
            
        grad_norm = torch.nn.utils.clip_grad_norm_([p for p in policy.parameters() if p.grad is not None], 1.0)
        optimizer.step()
        scheduler.step()
        print(f"[DEBUG train] Final rollout_loss to log: {rollout_loss:.6f}")
        train_step += 1
